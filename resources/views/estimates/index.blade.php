...
@section("body")
    ...
    <div role="tablist" class="tabs tabs-border border-b-base-300 border-b-1">
        ...
        >
            {{ __("estimate.open") }}
            <span class="badge badge-sm badge-neutral p-0">{{ $count }}</span>
        </a>
        <a
        ...
        <table class="table-zebra table" x-data>
            ...
            <tbody>
                @foreach ($estimates as $estimate)
                    <tr
                        x-link="{{ route("estimates.show", $estimate) }}"
                        ...
                        <td>
                            @switch($estimate->state)
                                @case(App\Domains\Estimates\Estimate::STATUS_DRAFT)
                                    <span class="badge badge-soft bg-base-300">
                                        {{ __("estimate.status.draft") }}
                                    </span>

                                    @break
                                @case(App\Domains\Estimates\Estimate::STATUS_SENT)
                                    <span class="badge badge-soft badge-info">
                                        {{ __("estimate.status.sent") }}
                                    </span>
