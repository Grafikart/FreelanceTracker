<table class="table table-zebra">
    ...
    <template x-for="(value, index) in rows" :key="index">
        <tr x-sort:item="value">
            <td>
                <div class="flex">
                    <button class="pr-4 cursor-move" x-sort:handle>
                    ...
                    <textarea x-grow class="textarea w-full min-h-auto" rows="1" type="text"
                              x-model="value.label"></textarea>
                </div>
            </td>
            <td><input class="input text-end" type="text" x-model="value.quantity"/></td>
            <td><input class="input text-end" type="text" x-model="value.price"/></td>
            <td>
                <div class="flex items-center gap-4">
                    <span x-text="formatPrice(value.price * value.quantity)"></span>
...
<div class="flex justify-between">
    ...
    <table class="text-end text-sm">
        ...
        <tr>
            <td class="pr-6">{{ __('estimate.tax') }}
                (<span x-text="tax"></span>%)</td>
            <td class="py-1" x-text="formatPrice(totalTax)"></td>
        </tr>

        <tr class="text-xl text-base-content font-bold">
