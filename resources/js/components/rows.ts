...
Alpine.data('rows', (props: {rows: Row[], tax: number, currency: string}) => ({
    rows: [] as Row[],
    tax: props.tax,
    ...
    reorder (e: {oldIndex: number, newIndex: number}) {
        ...
        const movedRow = this.rows.find(row => row.position === e.oldIndex)
        if (!movedRow) {
        ...
        if (e.newIndex > e.oldIndex) {
            this.rows
                .filter(r => r.position > e.oldIndex && r.position <= e.newIndex)
    ...
    get total () {
        return this.rows.reduce((acc, row) => acc + row.price * row.quantity, 0)
    },
